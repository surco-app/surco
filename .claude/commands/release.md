...
- Write every item from the USER's point of view, never the implementation's. Say what the reader gains, keeps, loses or stops having to redo — not what the code moves internally. Never name a structure the user cannot see: entry, record, field, path, pointer, internal identity, marker tree, header, probe, cached thumbnail, or the Traktor/rekordbox collection as a database. "Converting a track you keep in rekordbox points its entry at the new file" is WRONG; "convert a track you keep in rekordbox and it stays the same track: it keeps its playlists and its cue points" is right. When the change IS internal (a detector, a threshold, a codec), state the symptom that disappears, not the mechanism that fixed it, and drop engineering numbers (dB thresholds, low bits, ms offsets) unless they are evidence a reader weighs — a sweep size or a false-alarm count earns its place; a calibration constant does not.
