...
// `MetaTextKey` must list every key, so adding a field to TrackMetadata without
...
const FIELD_PRESENCE: Record<MetaTextKey, true> = {
  title: true,
  artist: true,
  album: true,
  albumArtist: true,
...
export const METADATA_KEYS = Object.keys(FIELD_PRESENCE) as MetaTextKey[]
...
export function emptyMetadata(): TrackMetadata {
  const blank = {} as Record<MetaTextKey, string>
