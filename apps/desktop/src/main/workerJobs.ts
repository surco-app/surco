...
import { type CueShift, copyCueFrames, dropFlacCueComment, writeTags } from './tags'
...
export type WorkerJob =
  ...
  // FLAC's cue blob survives the re-encode on its own; only a trim needs it gone.
  | { type: 'dropFlacCueComment'; file: string }
...
export function runWorkerJob(job: WorkerJob): WorkerJobResult | Promise<WorkerJobResult> {
  switch (job.type) {
    ...
    case 'copyCueFrames':
      ...
      return null
    case 'dropFlacCueComment':
      dropFlacCueComment(job.file)
      return null
    case 'prependFlacId3':
