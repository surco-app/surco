...
export async function runProcessTrack(
  ...
  try {
    ...
    if (musicOnly) {
      ...
      // set would have removeRenamedOriginal unlink a source the user asked to keep.
      // besideOriginal makes the same promise the same way (see resolveOutputTarget).
      if (choice === 'keepBoth') {
